...
def get_mappings(chromosome):
    ...
    # --- Relaxed (pos, ref) matching for gap score ---
    # Build (pos, ref) -> line_index maps (take first match if duplicates)
    pangenome_posref = {}
    for (pos, ref, alt), idx in pangenome_mapping.items():
        pr = (pos, ref)
        if pr not in pangenome_posref:
            pangenome_posref[pr] = idx

    thousand_g_posref = {}
    for (pos, ref, alt), idx in thousand_g_phased_mapping.items():
        pr = (pos, ref)
        if pr not in thousand_g_posref:
            thousand_g_posref[pr] = idx

    pangenome_to_thousand_g_posref = {}
    for pr in pangenome_posref:
        if pr in thousand_g_posref:
            pangenome_to_thousand_g_posref[pangenome_posref[pr]] = thousand_g_posref[pr]

    pg_mask_posref = []
    onek_mask_posref = []
    for key in pangenome_to_thousand_g_posref:
        pg_mask_posref.append(key)
        onek_mask_posref.append(pangenome_to_thousand_g_posref[key])
    pg_mask_posref = np.array(pg_mask_posref)
    onek_mask_posref = np.array(onek_mask_posref)

    np.save(f"{BASE_PATH}/starting_data/chr{chromosome}/pangenome_mask_posref.npy", pg_mask_posref)
    np.save(f"{BASE_PATH}/starting_data/chr{chromosome}/thousand_g_phased_mask_posref.npy", onek_mask_posref)

    print(f"Relaxed (pos,ref) mapping: {len(pangenome_to_thousand_g_posref)} positions (vs {len(pangenome_to_thousand_g_phased)} strict)")
