...
class HaplotypeHMM:
    def __init__(self, chromosome, subject_id):
        ...
        self.pangenome_haplotypes_without_subject = self.pangenome_without_subject.transpose(0,2,1).reshape(-1, self.pangenome_without_subject.shape[1])
    ...
    def get_transition_matrix_without_subject(self, pos_1, pos_2):
        ...
        return trans # Log transition probabilities
    ...
    def get_transition_probabilities(self, anchor_snps):
        ...
        for i in range(len(anchor_snps_positions) - 1):
            ...
            transition_prob = self.get_transitions(snp1_pos, snp2_pos)
    ...
    def forward_algorithm(self, haplotype, block_idx):
        ...
        alpha_previous = np.ones(num_states) / num_states
        log_alpha_previous = np.log(alpha_previous) + emissions[:, 0]

        # Compute transition matrices between anchor SNPs
        transition_probs = self.get_transition_probabilities(anchor_snps)

        log_alpha_current = None

        for t in range(1, num_snps):
            p,q = transition_probs[t - 1]  # log-transitions between t-1 and t

            p_alpha = log_alpha_previous + p  # no switch
            q_alpha = log_alpha_previous + q  # switch
            log_alpha_sum = logsumexp(log_alpha_previous)
            log_alpha_sum_q = log_alpha_sum + q  # switch from any state

            log_alpha_current = np.logaddexp(p_alpha, log_alpha_sum_q)
            log_alpha_current = logsubexp(log_alpha_current, q_alpha)  # combine no-switch and switch
            log_alpha_current += emissions[:, t]

            log_alpha_previous = log_alpha_current

        final_log_probs = log_alpha_previous
        total_log_prob = logsumexp(final_log_probs)
    ...
    def sample_block_prior(self, block_idx):
        ...
        for t in range(1, num_snps):
            ...
            probs = trans[sampled_states[t - 1]]
            probs /= probs.sum()
