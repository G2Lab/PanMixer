...
if [[ -s "${{OUTDIR}}/vg_idx.giraffe.gbz" ]]; then
  echo "vg_idx.giraffe.gbz already exists, skipping."
else
...
def vg_prep(experiment_number):
    ...
    return launch_job_with_custom_command_by_rowid("vg_prep", id_counter, command, memory="32g", cpus="16", num_tasks=str(num_tasks))
