...
from tools.common.utils import str_to_bool
...
def get_all_af_subsets(chromosome):
    ...
    for i in range(global_af.shape[0]):
        if np.any(np.isnan(global_af[i])):
            global_maf[i] = 1
            continue
        non_zero = global_af[i][global_af[i] > 0]
        if len(non_zero) == 0:
            global_maf[i] = 0
            continue
        mininum_non_zero = np.min(non_zero)
        maximum_non_zero = np.max(non_zero)
...
def compute_l1_loss_af_complex(original_haplotypes, new_haplotypes):
    ...
    #assert original_haplotypes.shape == new_haplotypes.shape, f"Shapes must match {original_haplotypes.shape} {new_haplotypes.shape}"
...
def af_loss_computer(experiment_number, dont_replace):
    ...
    return launch_job_multichromosome("af_loss", args, memory="32g", cpus="1", num_tasks=str(num_tasks))
...
def main():
    ...
    get_af_loss(experiment_number, chromosome, row_id, dont_replace)

def get_af_loss(experiment_number, chromosome, row_id, dont_replace):
    ...
    if not dont_replace:
        ...
        new_pangenome = np.load(EXPERIMENT_PATH + f"/exp_{experiment_number}/data/chr{chromosome}/{row_id}/new_haplotypes.npy")
        print(new_pangenome.shape)
