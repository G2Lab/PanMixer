...
from tools.common.VCFtoNP import get_numpy_matrices
import json
import sys
from tools.common.slurm_helper import launch_job
...
def align_beagle_to_positions(beagle_positions, beagle_haplotypes, target_positions, fill_value=-1):
    ...
    if missing_count > 0:
        print(f"Warning: {missing_count} out of {len(target_positions)} target positions were not found in Beagle output.")
